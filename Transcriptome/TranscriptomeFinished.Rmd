...
# This is making a data table in our environment using phenotypic data, it is organizing the identity of our 4 samples with their growth stage 
```{r}
...
# Filters ballgown data to remove all transcript data with variance less than one so that only informative transcripts remain 
```{r}
...
```
# The fold change value >1 means that this gene is being upregulated but the pvalue and qvalue tell us that the data is not statistically significant for the gene dnaA which corresponds with the RNA transcript gene-PA0001, feature and id tell us that this is the first transcript of the data set

# This is filtering out all transcripts that are not significant 
```{r}
sigdiff <- results_transcripts %>% filter(pval<0.05)
dim(sigdiff)
```

# organize the table, the table is being organized by increasing pvalues 
```{r}
...
# The head code gives us the first 6 rows of the gene_expression table, same as the above code, adding the dim code tells us the dimensions of this table (the number of columns and rows) to give us an idea of the scale of our dataset (4592 genes) 
```{r}
...
## There are up to 64 transcripts for a single gene which occurs because of alternative splicing, the large majority of the genes had 5 trancripts of less, only 587 out of 4715 genes had more than one transcript

# create a plot of how similar the two replicates are for one another, below this chunk another plot is made for biofilm replicates 
```{r}
...
## FPKM tells us about gene expression levels, if the tables are similar than the samples have similar expression of genes 
...
## The bars to the right of the dashed lines indicate genes are upregluated in planktonic cells while the bars to the left of the dashed lines indicate gens are upregulated in biofilm cells, the height of the bars tells us the amount of genes with each particular fold change value 
...
```{r}
ballgown::transcriptNames(bg_filt)[13]
ballgown::geneNames(bg_filt)[13]
```
# transform to log2
```{r}
transformed_fpkm <- log2(fpkm[13, ] + 1)
```
...
boxplot(transformed_fpkm ~ pheno_data$stage,
        main=paste(ballgown::geneNames(bg_filt)[13], ' : ', ballgown::transcriptNames(bg_filt)[13]),
        xlab="Stage",
        ylab="log2(FPKM+1)",
...
## There is higher gene expression of gene numbr 13 in the planktonic stage than in the biofilm stage 